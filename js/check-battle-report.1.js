$(function () {
  ...
  $('#list').table({
    ...
    $(this).closest('.mmGrid').find('th:eq(0) .mmg-title').text('次序');
  ...
  $('#list1').table({
    ...
    items: [
      {
        itemId: 'BFrmwFoNQGW0CJmVNwbziLqHDS8=',
        itemName: '<i>营业部</i>曹风国',// itemName 第二列 映射的值  i标签里面字体不一样
        wsHouseCount: 222, // 映射的值
        gtHouseCount: 103,  // 映射的值
        houseRate: 200.00,  // 映射的值
        wsTransferCount: 3000.22, // 映射的值
  ...
  $('.order-body').on('click', '.items', function (e) {
    ...
    $('.page-table').removeClass('dn').addClass('dn').eq($(this).index()).removeClass('dn');
    // var params = {
