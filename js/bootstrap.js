  ...
  + function ($) {
    ...
    Tree.prototype.createNode = function ($ul, data, level) {
      ...
      if (children && children.length > 0) {
        ...
        $li = $('<li><p class="branch" style="padding-left: ' + (level * 22 + 10)/100 + 'rem;"><label class="panel-checkbox"><input type="checkbox" data-id="' + id + '"><span class="name-val">' + name + '</span></label></p></li>')
        $li.find('input').on('click.bs.tree', this.outputVal.bind(this))
        $li.find('label').before($icon)
        $li.data('data', data)
        $ul.append($li)
      } else {
        $li = $('<li class="leaf-box"><p class="branch" style="padding-left: ' + (level > 1 ? level * 22 + 15.5 + 10 : level * 37 + 10)/100 + 'rem;"><label class="panel-checkbox"><input class="leaf" type="checkbox" data-id="' + id + '"><span class="name-val">' + name + '</span></label></p></li>')
        $li.find('input').on('click.bs.tree', this.outputVal.bind(this))
        $li.data('data', data)
        $ul.append($li)
        return $ul
...
+ function ($) {
  ...
  Select.prototype._renderFromData = function ($ctrl) {
    ...
    if (this.option.multiple && this.option.search) {
      ...
      this.$ul = $('<ul style="padding: 0; overflow: auto; height: 5.25rem;">')
  ...
  Select.prototype._renderFromUrl = function ($ctrl) {
    ...
    if (this.option.multiple) {
      ...
      this.$ul.append(temp)
      this.$ul.removeClass('select-menu').css({ padding: '0', overflow: 'auto', height: '5.25rem' })
      this.$select.append(this.$ul)
      this.renderFooter()
      this.el.append(this.$select)
    } else {
  ...
  Select.prototype.toggle = function (e) {
    ...
    if (isActive) {
      ...
      this.$ul.css('max-height', ((this.option.rows || 10) * 35 + 2)/100 + 'rem')
  ...
  function handleDirection($select, $ctrl, bottomHeight, topHeight, height) {
    if (bottomHeight < height && topHeight < height) {
      if (bottomHeight < topHeight) {
        ...
        $select.css('max-height', topHeight/100 + 'rem')
      } else {
      ...
      if (topHeight > height) {
        if (!$ctrl.hasClass('dropup')) $ctrl.addClass('dropup')
        $select.css('max-height', height/100 + 'rem')
      } else {
        if ($ctrl.hasClass('dropup')) $ctrl.removeClass('dropup')
        $select.css('max-height', height/100 + 'rem')
      }
  ...
  MMGrid.prototype = {
    ...
    _initLayout: function ($el) {
      ...
      var mmGrid = [
        '<div class="mmGrid">',
        '<style></style>',
        '<div class="mmg-headWrapper" style="width:auto">',
    ...
    _setColsWidth: function () {
      ...
      for (var colIndex = 0; colIndex < $ths.length; colIndex++) {
        ...
        styleText.push('width: ' + width/100 + 'rem;');
        styleText.push('max-width: ' + width/100 + 'rem;');
      ...
      for (var i = 0; i < cols.length; i++) {
        ...
        if (col.frozen) {
          ...
          if ($frozen && $frozen.length > 0) {
            ...
            $frozen = $(
              '<div class="mmg-frozen-' + name + ' mmg-frozen mmg-bodyWrapper" style="width: ' + (col.width + 1)/100 + 'rem;">' +
              '  <table class="mmg-frozen-head" style="width: ' + col.width/100 + 'rem;">' +
              '    <thead>' +
              '      <tr>' +
              '        <th class="mmg-frozen-head-th' + (col.sortable ? ' mmg-canSort' : '') + '"' + (col.checkCol ? ' style="padding: .2rem 0;"' : '') +
...
+(function (root, $) {
      ...
      if (nodes.length > 1) {
        $.each(nodes, function (i, m) {
          ...
          if (!r) {
            ...
            if (!scrolled) {
              $(scroller).animate({
                scrollTop: ($el.offset().top - scrollOffset)/100 + 'rem'
              })
              scrolled = true
            }
          } else {
  ...
  function addMagnifier($el, pattern) {
    ...
    setTimeout(function () {
      var pos = $el.position()
      $el.before('<div class="magnifier popover top" style="top:' + (pos.top - 45)/100 + 'rem; left: ' + pos.left/100 + 'rem;">' +
        '<div class="arrow"></div>' +
